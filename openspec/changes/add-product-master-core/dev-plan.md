...
| P5 前端 | ✅ 基本完成（7.10 待收尾） | 三个主数据列表、商品列表、档案页、5 步向导已在浏览器里用真实后端走通；前端 21 个规则测试通过；biome、tsc（本次改动 0 新增错误）、antd lint 通过 |
| P6 迁移与收尾 | 🟡 迁移完成，收尾待做 | 迁移脚本在 `scripts/product-migration/`：干跑、生成幂等 SQL 与回滚 SQL、导入后核对；测试库空库导入 87 商品 / 437 规格 / 82 FAQ / 56 参考价 / 117 应用场景，二次导入计数不变，回滚后全部软删除，逐项与独立站原文核对通过；12 个脚本测试通过。**尚未对开发库 `zhul_erp` 执行**。剩 7.10 与 9.3 |

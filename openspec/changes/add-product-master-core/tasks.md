...
- [ ] 7.10 **（进度 2026-09-20：axe 已扫描列表、档案页、向导、品牌页的深色与浅色主题，无违规；仅剩列表表头的 antd 内部 `ant-table-measure-row` 一处 aria-hidden-focus，属组件库内部节点；完整的键盘全流程手动走查、1024 以下的响应式还没做）** 按 `ui-design-patterns.md`「可访问性与规范补充」做可访问性验收：深色和浅色主题的对比度（正文 ≥4.5:1、控件边界 ≥3:1）、可见焦点与键盘全流程（含向导和弹窗）、交互目标 ≥24×24、`prefers-reduced-motion`、`aria-sort`、拖拽替代按钮、等宽数字，验证：用 axe 或 Lighthouse 扫描列表、向导、档案页三类页面无严重问题，并手动走查键盘流程

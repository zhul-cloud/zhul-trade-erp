...
- [x] 9.3 场景到测试的对应关系登记在 `scenario-coverage.json`，`node openspec/changes/add-product-master-core/check-scenario-coverage.js` 逐条核对（118 个场景全部有对应测试且方法真实存在），测试本身通过与否看 `mvn test`（463 个全部通过）。完成定义检查，验证：`specs` 下 5 份规格的全部 scenario 均有对应通过的测试；`git diff` 确认没有修改任何 v1.0 / v1.1 已有表；写接口经过权限码与 `PlatformScopeGuard` 双重校验；任务 1.1 已完成；`openspec validate add-product-master-core --strict` 通过

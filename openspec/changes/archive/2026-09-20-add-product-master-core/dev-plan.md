...
| P5 前端 | ✅ 完成 | 三个主数据列表、商品列表、档案页、5 步向导已在浏览器里用真实后端走通；前端 21 个规则测试通过；biome、tsc（本次改动 0 新增错误）、antd lint 通过 |
| P6 迁移与收尾 | ✅ 完成 | 迁移脚本在 `scripts/product-migration/`：干跑、生成幂等 SQL 与回滚 SQL、导入后核对；测试库空库导入 87 商品 / 437 规格 / 82 FAQ / 56 参考价 / 117 应用场景，二次导入计数不变，回滚后全部软删除，逐项与独立站原文核对通过；12 个脚本测试通过。已对开发库 `zhul_erp` 执行并核对通过（2026-09-20）。9.3 完成定义检查已完成 |

后端全量 463 个测试通过。开发库 `zhul_erp` 已执行 `schema_v1.2.sql`、`data_v1.2.sql` 和迁移脚本。

-- V1.0.1：系统结构性参考数据（菜单资源树、内置角色、内置字典）
-- 由旧的 sql/build/data/data_v1.sql 拆分而来：这部分是应用运行所依赖的结构性数据
-- （菜单权限树、内置角色、内置字典枚举），任何环境（含生产）都需要，所以放进
-- db/migration 随 schema 一起自动执行。
-- 示例租户 / 示例管理员账号等"仅供本地体验"的部分见 db/dev-data。
